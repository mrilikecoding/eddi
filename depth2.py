...
pathL = "data/stereoL/"
pathR = "data/stereoR/"
imgL = cv2.imread(pathL+"img0.png",0)
imgR = cv2.imread(pathR+"img0.png")
# Read both images and convert to grayscale
# ------------------------------------------------------------
# PREPROCESSING
imgL = cv2.medianBlur(imgL,9)
imgR = cv2.medianBlur(imgL,9)
# imgL = cv2.adaptiveThreshold(imgL,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
#             cv2.THRESH_BINARY,11,2)
# imgR = cv2.adaptiveThreshold(imgR,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
#             cv2.THRESH_BINARY,11,2)
# ret,th1 = cv2.threshold(imgL,127,255,cv2.THRESH_BINARY)
...
#     imgL, kp1, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
# cv2.imshow("Step 1: SIFT Keypoints", imgSift)
# cv2.waitKey(0)
...
matches = flann.knnMatch(des1, des2, k=2)
 ...
 # Create matcher
#  https://stackoverflow.com/questions/37734570/i-got-bad-matching-results-with-opencv-and-sift-how-could-i-improve-it
...
def drawlines(img1src, img2src, lines, pts1src, pts2src):
    ...
    img1color = np.copy(img1src)
    img2color = np.copy(img2src)
